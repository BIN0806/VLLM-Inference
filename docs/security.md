...
- First contact (`make ssh-scan-host`) captures a **candidate** key in a temporary file, prints the SHA256 fingerprint, and installs it only after `EXPECTED_FINGERPRINT=SHA256:...` or `CONFIRM=yes`. It does not append `ssh-keyscan` output to `known_hosts` as if it were already verified. Keys are stored in the gitignored project file `.ssh/known_hosts` so Vast rentals do not pollute `~/.ssh/known_hosts`.
