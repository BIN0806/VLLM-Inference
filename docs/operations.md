...
- Phase 4A (two-replica-capable StatefulSet + Prometheus scrape, KEDA not installed): [k3s-replicas-prometheus.md](runbooks/k3s-replicas-prometheus.md)
- Diagnostics: `make diagnostics` (local, redacted)
- Remote sync (after startup): `make sync-remote` with `INFERENCE_ALLOW_REMOTE=1`

Do not stop, restart, or kill a loading vLLM process. Phase 4A may install
Prometheus scrape on an approved two-GPU VM. Do not install KEDA, expose
public ports, scale to two replicas, or attempt scale-to-zero until those
gates are approved.
